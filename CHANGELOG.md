...
- Merged the doctor-photo placeholder update to `main` as `3efe92d`; the designated Railway web service deployed it successfully. Live validation confirmed the neutral non-image placeholder renders in the About the Dentist section without horizontal overflow.
- Merged the expanded protected admin operations workspace to `main` as `63d3b0e`; Railway deployed it successfully. Production-boundary validation confirmed a representative public LP remains available while `/admin` and the new preview endpoint require authentication.
- Merged the five copy-based patient landing-page families and protected Copy LPs admin inventory to `main` as `fc6f09d`; Railway deployed it successfully. Live validation confirmed 15 practice LPs, 30 matched outcomes, assigned surveys, guide icons, approved consultation actions, and the protected admin boundary.
- Merged the supplied transparent moon-badge logo and two-loop copy-family refinement to `main` as `ea6487e`; Railway deployed it successfully. Live validation confirmed the borderless upper-left logo frame, retained copy-family guide icon, refined patient copy, and the assigned Pantego Dental survey.
