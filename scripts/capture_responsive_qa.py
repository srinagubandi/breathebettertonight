        ...
        with sync_playwright() as playwright:
            ...
            for name, route, survey_id, practice_key in routes:
                ...
                for label, context_options in (("mobile", MOBILE), ("desktop", DESKTOP)):
                    ...
                    if practice_key == 'pantego-dental':
                        approved_portrait = page.locator('.dentist-photo-approved img')
                        if not approved_portrait.count() or approved_portrait.get_attribute('src') != '/assets/images/providers/dr-willis-lay.jpeg':
                            raise AssertionError(f"Approved Dr. Lay portrait missing in {name} {label}")
                    elif not page.locator('.dentist-photo-placeholder').count():
