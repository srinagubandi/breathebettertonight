  ...
  const body = `<div class="landing-v3 landing-${escapeHtml(campaign.designSystem || 'night-to-clarity')}">
    <section class="landing-hero" style="--hero-image:url('${escapeHtml(campaign.hero)}')">
      ...
      <div class="landing-container landing-hero-content">
        <p class="landing-eyebrow">${escapeHtml(campaign.eyebrow || 'Night-to-Clarity')}</p>
        <h1>${escapeHtml(campaign.headline)}</h1>
        <p class="landing-lede">${escapeHtml(campaign.subheadline)}</p>
        <div class="landing-actions"><a class="landing-button landing-button-primary" href="#free-consultation">Request a free consultation <span aria-hidden="true">→</span></a><a class="landing-button landing-button-secondary" href="tel:${escapeHtml(practice.phoneRaw)}">Call ${escapeHtml(practice.phoneDisplay)}</a></div>
        <p class="landing-microcopy">No diagnosis. No pressure. A practical next step.</p>
      </div>
    </section>
