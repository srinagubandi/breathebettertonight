 ...
 */
function renderChatLauncher({ phone, phoneRaw, showPhone = true }) {
  ...
  const fallbackCopy = showPhone && phoneRaw
    ? 'Use this space to connect a compliant chat provider when you are ready. For now, the office can be reached by phone.'
    : 'Use this space to connect a compliant chat provider when you are ready.';
  const callAction = showPhone && phoneRaw ? `<a class="chat-call-link" href="tel:${phoneRaw}">Call ${phone}</a>` : '';
  ...
  return `
    <section class="chat-widget" aria-label="Chat support">
      ...
      <div class="chat-panel" id="chat-panel" role="dialog" aria-modal="false" aria-labelledby="chat-panel-title" hidden>
        ...
        <p class="chat-panel-copy">${fallbackCopy}</p>
        ${providerAction}
        ${callAction}
